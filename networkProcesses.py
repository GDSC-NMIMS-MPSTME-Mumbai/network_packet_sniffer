...
class NetworkProcessWorker(QThread):
    ...
    def getConnection():
        ...
        )  # connect and take care of big endian <-> little endian
        return connection
    ...
    def get_ethernet_frame(self, data):  # on receiving a packet
        destination_mac, source_mac, protocol = struct.unpack(
            "! 6s 6s H", data[:14]
        )  # 6 bytes for source and dest, H is small unsigned int, total 14
        return (
            self.get_mac_address(destination_mac),
    ...
    def ipv4_packet(self, data):
        ...
        time_to_live, protocol, source, target = struct.unpack(
        ...
        return (
            ...
            time_to_live,
            protocol,
            self.get_ip_address(source),
            self.get_ip_address(target),
            data[header_length:],
        )
    ...
    def run(self):
        ...
        while True:
            ...
            (
                ...
                ethernet_protocol,
                data,
            ) = self.get_ethernet_frame(raw_data)
            print(
            ...
            if ethernet_protocol == 8:
                ...
                print("IPv4 Packet:")
                print(
                    f"Version: {version}, Header Length: {header_length}, Time To Live: {time_to_live}, Protocol: {protocol}, Source: {source}, Target: {target} "
                )
                # ICMP
                if protocol == 1:
                    ...
                    self.packet.emit(
                        (
                            ...
                            source,
                            target,
                            f"Type: {icmp_type}, Code: {code}, Checksum: {checksum}, Data: {data}",
                        )
                    )
                ...
                if protocol == 6:
                    (
                        ...
                        flag_syn,
                        data,
                    ) = self.tcp_segment(data)
                    print("TCP Segment:")
                    print(
                        f"Source Port: {source_port}, Destination Port: {destination_port}\nSequence: {sequence}, Acknowledgement: {acknowledgement}"
                    )
                    print(
                        f"URG: {flag_urg}, ACK: {flag_ack}, FIN: {flag_fin}, PSH: {flag_psh}, RST: {flag_rst}, SYN: {flag_rst}, SYN: {flag_syn}"
                    )
                    print(f"Data:\n{data}")
                    ...
                    self.packet.emit(
                        (
                            ...
                            source_port,
                            destination_port,
                            f"{data}, Sequence: {sequence}, Acknowledgement: {acknowledgement}, URG: {flag_urg}, ACK: {flag_ack}, FIN: {flag_fin}, PSH: {flag_psh}, RST: {flag_rst}, SYN: {flag_rst}, SYN: {flag_syn}",
                        )
                    )
                    ...
                    print("UDP Segment:")
                    print(
                        f"Source Port: {source_port}, Destination Port: {destination_port}, Length: {size}"
                    )
